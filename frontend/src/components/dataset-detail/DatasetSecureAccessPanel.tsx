...
export default function DatasetSecureAccessPanel({
    ...
    const accessStateLabel = escrowActive
        ? 'Escrow active'
        : startEscrowGuardrail.allowed
          ? 'Ready'
          : 'Approval gated'
    const accessStateClasses = escrowActive
    ...
    return (
        <DatasetDetailPanel
            ...
            badge={
                <span className={`rounded-sm border px-2.5 py-1 text-[10px] font-semibold ${accessStateClasses}`}>
                    {accessStateLabel}
                </span>
            }
            action={
        ...
        >
            <div className="space-y-3">
                <p className={`text-xs leading-5 ${startEscrowGuardrail.allowed ? 'text-slate-300' : 'text-amber-300'}`}>
                    {startEscrowGuardrail.allowed
                ...
                {expanded ? (
                    <div className="space-y-3">
                        <div className="grid grid-cols-1 gap-3">
                            <div className="rounded-sm border border-emerald-500/25 bg-slate-950/60 p-3">
                            ...
                            <div className="rounded-sm border border-slate-700/70 bg-slate-950/40 p-3">
                                ...
                                <button
                                    type="button"
                                    className="w-full rounded-sm border border-slate-600 px-3 py-2.5 text-sm text-slate-200 transition-colors hover:border-slate-400 hover:text-white"
                                >
